...
void SpaceScene::Init()
{
	...
	glUniformMatrix4fv(glGetUniformLocation(glyphShader.ID, "projection"), 1, GL_FALSE, glm::value_ptr(glyphProjMatrix));
...
void SpaceScene::Render()
{
	...
	RenderText(glyphShader, "This is sample text", 25.0f, 25.0f, 1.0f, glm::vec3(0.5, 0.8f, 0.2f));
	glUseProgram(0);
...
void SpaceScene::RenderText(Shader &shader, std::string text, float x, float y, float scale, glm::vec3 color)
{
